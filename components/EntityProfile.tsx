...
const PitchPlayerDot: React.FC<{
  ...
  return (
    <div className="flex flex-col items-center gap-0.5 group relative">
      {/* Tooltip */}
      <div className="absolute bottom-full mb-2 hidden group-hover:block z-50 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="w-2 h-2 bg-dark-700 border-r border-b border-dark-600 rotate-45 absolute -bottom-1 left-1/2 -translate-x-1/2"></div>
      </div>
...
const SofaScorecard: React.FC<{
  ...
  return (
    <div className="bg-dark-800 rounded-2xl border border-dark-700 overflow-hidden shadow-2xl">
      {/* League header bar */}
      <div className="bg-dark-900/80 px-6 py-3 flex items-center justify-between border-b border-dark-700">
        <div className="flex items-center gap-2">
          <Shield size={14} className="text-gray-400" />
          <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">{match.league}</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase flex items-center gap-1.5 ${
            ...
            match.status === 'FT' ? 'bg-gray-700/50 border border-gray-600/50 text-gray-300' :
      ...
      <div className="px-6 py-8 md:py-10">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          ...
          <div className="flex-1 flex flex-col items-center gap-3">
            <button
              ...
              className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-dark-700 border-2 border-dark-600 flex items-center justify-center hover:border-pitch-500 transition group"
            >
          ...
          <div className="flex flex-col items-center gap-2 px-6">
            {isUpcoming ? (
              <div className="flex flex-col items-center">
                ...
                <span className="text-xs text-gray-500 mt-2">{match.minute}</span>
              </div>
            ) : (
              <>
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-2xl md:text-3xl font-light text-gray-600">-</span>
                  <span className={`text-5xl md:text-6xl font-black tabular-nums ${
                    isLive ? 'text-white' : 'text-white'
                  }`}>{awayScore}</span>
                </div>
                {match.status === 'HT' && (
                  <span className="text-[10px] text-gray-500 font-bold uppercase">Half Time</span>
                )}
              </>
            )}
          </div>
          ...
          <div className="flex-1 flex flex-col items-center gap-3">
            <button
              ...
              className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-dark-700 border-2 border-dark-600 flex items-center justify-center hover:border-blue-500 transition group"
            >
        ...
        {/* Key events under score */}
        {match.events && match.events.length > 0 && (
          <div className="mt-6 pt-4 border-t border-dark-700 max-w-2xl mx-auto">
              ...
              <div className="flex-1 flex flex-col gap-1">
                {match.events.filter(e => e.team === 'HOME' && e.type === 'GOAL').map((ev, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs text-gray-400">
                    ...
                    <span className="text-gray-600 font-mono text-[10px]">{ev.minute}</span>
                  </div>
                ))}
              </div>
              {/* Away goals */}
...
// ============================================================================
// LIST-STYLE LINEUP ROW (SofaScore sidebar)
// ============================================================================
...
const LineupListRow: React.FC<{ player: LineupPlayer; isHome: boolean; idx: number }> = ({ player, isHome, idx }) => {
  ...
  return (
    <div className={`flex items-center gap-3 py-2 px-3 hover:bg-white/5 transition rounded ${!isHome && 'flex-row-reverse'}`}>
      ...
      <div className={`flex-1 min-w-0 ${!isHome && 'text-right'}`}>
        ...
        <div className={`flex items-center gap-2 text-[9px] text-gray-500 ${!isHome && 'justify-end'}`}>
...
const SofaScoreEmbed: React.FC<{ homeTeam: string; awayTeam: string }> = ({ homeTeam, awayTeam }) => {
  ...
  return (
    <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-dark-900/60 border-b border-dark-700">
        <div className="flex items-center gap-2">
          <img src="https://www.sofascore.com/favicon.ico" alt="SofaScore" className="w-4 h-4" />
          <span className="text-xs font-bold text-gray-300">SofaScore</span>
        </div>
        <a
          href={sofascoreUrl}
          target="_blank"
...
const EntityProfile: React.FC<EntityProfileProps> = ({ entity, reviews, onRate, onUserClick, onAddToPlaylist, onTeamClick }) => {
  ...
  if (match) {
    ...
    return (
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 max-w-4xl mx-auto px-4">
        {/* Scorecard */}
        <SofaScorecard
          match={match}
          isLive={isLive}
        ...
        <div className="flex items-center justify-center gap-3 mt-4">
          ...
          <button
            ...
            className="bg-dark-800 hover:bg-dark-700 text-gray-200 font-bold py-2 px-5 rounded-lg border border-dark-700 flex items-center gap-2 transition text-sm"
          >
        ...
        {/* Tabs */}
        <div className="mt-6 flex items-center gap-1 bg-dark-800 rounded-xl p-1 border border-dark-700 overflow-x-auto">
        ...
        <div className="mt-6 space-y-6">
          ...
          {activeTab === 'summary' && (
            <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[9px] text-gray-500">{Math.round(mlPrediction.confidence * 100)}% confidence</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl font-black text-white">
                        {mlPrediction.score.toFixed(1)}<span className="text-xs text-gray-500">/10</span>
                      </div>
                      <div className={`text-[9px] font-bold uppercase mt-0.5 ${
                        mlPrediction.score >= 8 ? 'text-orange-400' : mlPrediction.score >= 6 ? 'text-purple-400' : 'text-gray-400'
                      }`}>
              ...
              {hasLineups && (
                <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700 flex items-center justify-between">
                    <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Formation</span>
                    <button
              ...
              {hasEvents && !isUpcoming && (
                <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700">
                    <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Key Events</span>
                  </div>
                      ...
                      <div key={i} className={`flex items-center gap-3 px-4 py-3 ${ev.team === 'AWAY' ? 'flex-row-reverse' : ''}`}>
                        <div className={`flex items-center gap-2 flex-1 min-w-0 ${ev.team === 'AWAY' ? 'justify-end text-right' : ''}`}>
                          ...
                          <div className="min-w-0">
                            ...
                            {ev.playerOut && <div className="text-[9px] text-gray-500">Off: {ev.playerOut}</div>}
                          </div>
                        </div>
                        <div className="w-10 h-10 rounded-full bg-dark-900 border border-dark-700 flex items-center justify-center font-mono font-bold text-pitch-500 text-[11px] shrink-0">
                          {ev.minute}
                        </div>
              ...
              {/* Community reviews */}
              <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700 flex items-center gap-2">
                  <Users size={14} className="text-pitch-500" />
                  <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Reviews</span>
                </div>
                <div className="p-4">
                  {reviews.length > 0 ? (
          ...
          {activeTab === 'lineups' && hasLineups && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700 flex items-center justify-between">
                  <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Pitch View</span>
                ...
                <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-gray-500 font-mono">{match.formation?.home}</span>
                    </div>
                  </div>
                  ...
                  {match.bench?.home && match.bench.home.length > 0 && (
                    <>
                      <div className="px-4 py-2 bg-dark-900/30 border-t border-dark-700">
                        <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Substitutes</span>
                      </div>
                      <div className="divide-y divide-dark-700/50 opacity-70">
                ...
                <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700">
                    <div className="flex items-center gap-2 justify-end">
                      <span className="text-[10px] text-gray-500 font-mono">{match.formation?.away}</span>
                  ...
                  {match.bench?.away && match.bench.away.length > 0 && (
                    <>
                      <div className="px-4 py-2 bg-dark-900/30 border-t border-dark-700">
                        <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider text-right block">Substitutes</span>
                      </div>
                      <div className="divide-y divide-dark-700/50 opacity-70">
          ...
          {activeTab === 'stats' && matchStats && (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700">
                  <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Match Statistics</span>
                </div>
                    ...
                    return (
                      <div key={i}>
                        ...
                        <div className="flex gap-1 h-2">
                          <div className="flex-1 bg-dark-700 rounded-full overflow-hidden flex justify-end">
                            <div
                              className="h-full bg-pitch-500 rounded-full transition-all duration-500"
                              style={{ width: `${homePercent}%` }}
                            />
                          </div>
                          <div className="flex-1 bg-dark-700 rounded-full overflow-hidden">
                            <div
          ...
          {activeTab === 'timeline' && hasEvents && !isUpcoming && (
            <div className="animate-in fade-in duration-300">
              <div className="bg-dark-800 border border-dark-700 rounded-xl overflow-hidden">
                <div className="px-4 py-3 bg-dark-900/50 border-b border-dark-700">
                  <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Match Timeline</span>
                </div>
                <div className="relative py-4">
                  {/* Center line */}
                  <div className="absolute left-1/2 top-0 bottom-0 w-px bg-dark-700"></div>
                  ...
                  {match.events?.map((ev, i) => (
                    <div key={i} className={`flex items-center gap-3 px-4 py-2 ${ev.team === 'AWAY' ? 'flex-row-reverse' : ''}`}>
                      <div className={`flex-1 min-w-0 ${ev.team === 'AWAY' ? 'text-right' : ''}`}>
                        <div className={`inline-flex items-center gap-2 bg-dark-900 border border-dark-700 rounded-lg px-3 py-2 ${ev.team === 'AWAY' ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className="relative z-10 w-10 h-10 rounded-full bg-dark-800 border-2 border-dark-600 flex items-center justify-center font-mono font-bold text-pitch-500 text-[11px] shadow-lg shrink-0">
                        {ev.minute}
                      </div>
  ...
  // ============================================================================
  // NON-MATCH ENTITY VIEW (Player / Team) - Keep existing
  // ============================================================================
  return (
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 text-center md:text-left pt-6">
            <div className="flex flex-col md:flex-row items-center md:items-end gap-3 mb-2">
              ...
              <span className="text-gray-400 font-light text-lg">{entity.league}</span>
            </div>
